...
class CrossCorrelationProbe(TemplateProbe):
    ...
    def __init__(self, molds, tag="target"):
        if not isinstance(molds, (tuple, list)):
            raise ValueError(
                "Argument 'mold' must be a list of " "time/frequency media."
            )
    ...
    def _build_output(self, corr, target, geom):
        ...
        if corr_values.size > 0:
            return {
                "geometry": geom,
                "labels": [
                    {"key": "tag", "value": self.tag, "type": "crosscorr_tag"}
                ],
                "score": {"peak_corr": np.amax(corr_values)},
            }
        return None
    ...
    def _extend_interval_with(self, mold):
        ...
        if self._frequency_interval is None:
            self._frequency_interval = FrequencyInterval(
    ...
    def apply(
        ...
        if limit_freqs:
            target = target.cut(
                min_freq=limit_freqs.min_freq, max_freq=limit_freqs.max_freq
            )
        corr = self.corr(target, method=method)

        all_peaks = peak_local_max(
            corr, min_distance=peak_distance, threshold_abs=thresh
        )
        ...
        for x, y in all_peaks:
            ...
            if xind2 - xind1 > 1:
                ...
                if yind2 - yind1 > 1:
                    ...
                    new_box = BBox(start_time, end_time, min_freq, max_freq)
    ...
    def shape_range(self):
        ...
        """
        return (
            np.amin(np.array([x.shape for x in self.template]), axis=0),
