...
def insert_datastore(dstore_config, col_config):
    ...
    with db_session:
        (
            ...
            recording_inserts,
            annotation_inserts,
        ) = datastore.insert_into(col)

    col.db_manager.db.disconnect()

    return {
        "datastore_record": datastore_id,
...
@transition(
    name="add_hash",
    outputs=["hashed_soundscape"],
    keep=True,
    persist=True,
    is_output=True,
    signature=(
        (DaskDataFramePlace, PickleablePlace, ScalarPlace),
        (DaskDataFramePlace,),
    ),
)
def add_hash(dataframe, hasher, out_name="xhash"):
    ...
    if not hasher.validate(dataframe):
        ...
        message = (
            "Input dataframe is incompatible with hasher."
...
@transition(
    name="as_dd",
    outputs=["recordings_dd"],
    signature=((PandasDataFramePlace, ScalarPlace), (DaskDataFramePlace,)),
)
def as_dd(pd_dataframe, npartitions):
    ...
    dask_dataframe = dd.from_pandas(
        pd_dataframe, npartitions=npartitions, name="as_dd"
    )
