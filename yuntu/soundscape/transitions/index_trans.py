...
def feature_slices(row, audio, config, indices):
    ...
    cuts, weights = slice_windows(
        config["time_unit"],
        audio.duration,
        config["frequency_bins"],
        config["frequency_limits"],
    )
    feature = getattr(audio.features, config["feature_type"])(
...
@transition(
    name="slice_features",
    outputs=["feature_slices"],
    persist=True,
    signature=(
        (DaskDataFramePlace, DictPlace, PickleablePlace),
        (DaskDataFramePlace,),
    ),
)
def slice_features(recordings, config, indices):
    ...
    meta += [(index.name, np.dtype("float64")) for index in indices]

    result = recordings.audio.apply(
        feature_slices, meta=meta, config=config, indices=indices
    )
...
@transition(
    name="apply_indices",
    outputs=["index_results"],
    is_output=True,
    persist=True,
    keep=True,
    signature=((DaskDataFramePlace, PickleablePlace), (DaskDataFramePlace,)),
)
def apply_indices(slices, indices):
    ...
    if len(index_names) != len(set(index_names)):
        message = (
            "Index names have duplicates. Please use a diferent name"
    ...
    results = slices.apply(feature_indices, meta=meta, axis=1, indices=indices)
    for index in indices:
