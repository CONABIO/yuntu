...
class DatastoreLoad(Pipeline):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not isinstance(admin_config, dict):
            raise ValueError("Argument 'admin_config' must be a dictionary.")
        if not isinstance(datastore_configs, list):
            raise ValueError("Argument 'datastore_configs' must be a list.")
    ...
    def build(self):
        self["init_config"] = place(
            self.collection_config, "dict", "init_config"
        )
        self["admin_config"] = place(
            self.admin_config, "dynamic", "admin_config"
        )
        self["datastore_configs"] = place(
            self.datastore_configs, "dynamic", "datastore_configs"
        )

        self["col_config"] = pg_init_database(
            self["init_config"], self["admin_config"]
        )
        self["insert_results"] = load_datastores(
            self["col_config"], self["datastore_configs"]
        )
...
class DatastoreLoadPartitioned(Pipeline):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not isinstance(datastore_config, dict):
            raise ValueError("Argument 'datastore_configs' must be a dict.")
    ...
    def build(self):
        ...
        self["datastore_config"] = place(
            self.datastore_config, "dynamic", "datastore_config"
        )
        self["npartitions"] = place(1, "scalar", "npartitions")
        self["rest_auth"] = place(self.rest_auth, "dynamic", "rest_auth")
        self["datastore_configs"] = source_partition(
            self["datastore_config"], self["rest_auth"], self["npartitions"]
        )
        self["col_config"] = pg_init_database(
            self["init_config"], self["admin_config"]
        )
