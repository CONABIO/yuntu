...
def getWindowPieces(window, spec, freqs, duration, limits, slice_dim, truncate=True):
    ntargets = len(limits)
    fmax = np.amax(freqs)
    fmin = np.amin(freqs)
    ...
    for i in range(ntargets):
        ...
        fbin1 = fbins
        if slice_dim == 2:
            fbin0 = int(max(math.floor(((min_freq-fmin)/freq_range)*fbins), 0))
            fbin1 = int(min(math.ceil(((max_freq-fmin)/freq_range)*fbins), fbins-1))
        if window is not None:
            if tbin1 - tbin0 < win_tsize:
                if tbin0 - half_win_t >= 0 and tbin1 + (win_tsize - half_win_t) <= tbins:
                    tbin0 = int(tbin0 - half_win_t)
                    tbin1 = int(tbin1 + (win_tsize - half_win_t))
                elif tbin0 - half_win_t >= 0:
                    tbin0 = int(max(0, int(tbin0 - half_win_t + (tbin1 + (win_tsize - half_win_t) - tbins ))))
            ...
            if slice_dim == 2:
                if fbin1 - fbin0 < win_fsize:
                    if fbin0 - half_win_f >= 0 and fbin1 + (win_fsize - half_win_f) <= fbins:
                        fbin0 = int(fbin0 - half_win_f)
                        fbin1 = int(fbin1 + (win_fsize - half_win_f))
                    elif fbin0 - half_win_f >= 0:
                        fbin0 = int(max(0, int(fbin0 - half_win_f + (fbin1 + (win_fsize - half_win_f) - fbins ))))
...
def getCoords(x, tbins, fbins, duration, freq_range, fmin, fmax):
    tbin0 = min(tbins-1, int(max(round((x[0]/duration)*tbins), 0)))
    fbin0 = min(fbins-1, int(max(round(((x[1]-fmin)/freq_range)*fbins), 0)))
    return [tbin0, fbin0]

def drawVect(mask, verts, tbins, fbins, duration, freq_range, fmin, fmax):
