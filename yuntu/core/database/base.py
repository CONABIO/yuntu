...
from yuntu.core.database.images import build_base_image_model
from yuntu.core.database.images import build_timed_image_model
...
#        """Construct the recording entity."""
#        return build_base_image_model(self.db)
...
#        return build_base_recording_model(self.db)
    ...
    @db_session
    def select(self, query, model="image"):
        """Query entries from database."""
        model_class = self.get_model_class(model)
    ...
    @db_session
    def delete(self, query, model="image"):
        """Delete entries using filter."""
        model_class = self.get_model_class(model)
        return [obj.delete() for obj in model_class.select(query)]

    @db_session
    def update(self, query, set_obj, model="image"):
        """Update matches."""
        model_class = self.get_model_class(model)
        return [obj.set(**set_obj) for obj in model_class.select(query)]

    @db_session
    def insert(self, meta_arr, model="image"):
        """Directly insert new media entries without a datastore."""
        model_class = self.get_model_class(model)
        return [model_class(**meta) for meta in meta_arr]
...
class TimedImageDatabaseManager(ImageDatabaseManager):

    def build_image_model(self):
        image = super().build_image_model()
        return build_timed_image_model(image)
