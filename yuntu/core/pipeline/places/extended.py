    ...
    def __array_ufunc__(self, ufunc, method, *inputs, **kwargs):
        ...
        for item in inputs:
            if isinstance(item, Node):
                ...
                op_inputs.append(
                    input_class(
                        name=f"{ufunc.__name__}" + f"_input_{arg_count}",
        ...
        for key in kwarg_keys:
            if isinstance(kwargs[key], Node):
                ...
                extra_args.append(
                    input_class(
                        name=f"{key}", pipeline=self.pipeline, data=kwargs[key]
                    )
                )
        ...
        if len(op_outputs) == 0:
            outputs.append(
                NumpyArrayPlace(
                    name=f"{transition_name}_output",
                    pipeline=self.pipeline,
                    is_output=False,
                    persist=False,
                    keep=False,
...
def _build_method(method_name, op_class, out_sig):
    def class_method(self, *args, **kwargs):
        return _build_method_op(
            self, method_name, args, kwargs, op_class, out_sig
        )
...
def _build_method_op(self, method_name, args, kwargs, op_class, out_sig):
    ...
    for item in args:
        if isinstance(item, Node):
            ...
            op_args.append(
                input_class(
                    name=f"{method_name}" + f"_input_{arg_count}",
                    pipeline=self.pipeline,
                    data=item,
                )
            )
    ...
    for out_spec in out_sig:
        ...
        outputs.append(
            out_class(
                name=(f"{transition_name}" + f"_output_{curr_len}"),
                pipeline=self.pipeline,
                is_output=False,
                persist=False,
                keep=False,
...
for meth in NUMPY_ARRAY_METHOD_SPECS:
    setattr(
        ...
        meth,
        _build_method(
            meth, Transition, NUMPY_ARRAY_METHOD_SPECS[meth]["out_sig"]
        ),
    )
...
for meth in DASK_SERIES_METHOD_SPECS:
    setattr(
        ...
        meth,
        _build_method(
            meth, Transition, DASK_SERIES_METHOD_SPECS[meth]["out_sig"]
        ),
    )
...
for meth in DASK_DATAFRAME_METHOD_SPECS:
    setattr(
        ...
        meth,
        _build_method(
            meth, Transition, DASK_DATAFRAME_METHOD_SPECS[meth]["out_sig"]
        ),
    )
...
for meth in DASK_SERIES_GROUPBY_METHOD_SPECS:
    setattr(
        DaskSeriesGroupByMixin,
        meth,
        _build_method(
            meth, Transition, DASK_SERIES_GROUPBY_METHOD_SPECS[meth]["out_sig"]
        ),
    )
