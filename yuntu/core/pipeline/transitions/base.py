    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not isinstance(inputs, (list, tuple)):
            message = (
                "Argument 'inputs' must be a list or a " + "tuple of nodes."
            )
            raise ValueError(message)
        ...
        if not isinstance(outputs, (list, tuple)):
            message = (
                "Argument 'outputs' must be a list or a " + "tuple of nodes."
            )
            raise ValueError(message)
        ...
        if is_output is not None:
            ...
            for val in is_output:
                if not isinstance(val, bool):
                    raise ValueError(
                        "All elements of 'is_output' must be" + " of type bool."
                    )
        ...
        if persist is not None:
            ...
            for index, node in enumerate(outputs):
                ...
                if not isinstance(node, self.signature[1][index]):
                    message = (
                        f"Output at position {index} is not" + " compatible."
                    )
                    raise ValueError(message)
        ...
        if len(inputs) > 0:
            ...
            for key in ikeys:
                if key not in self.pipeline.nodes_up[self.key]:
                    message = (
                        f"Unknown input {key} for transition " + f"{self.key}."
                    )
                    raise ValueError(message)
