    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.validate_operation(operation):
            raise ValueError(
                f"Operation type {type(operation)} incorrect "
                + "for this type of operation node."
            )

        if not isinstance(inputs, (list, tuple)):
            message = "Argument 'inputs' must be a list or a tuple of nodes."
            raise ValueError(message)
        ...
        if not isinstance(outputs, (list, tuple)):
            message = "Argument 'outputs' must be a list or a tuple of nodes."
            raise ValueError(message)
        ...
        if is_output is not None:
            if not isinstance(is_output, (bool, list, tuple)):
                raise ValueError(
                    "Argument 'is_output' must be bool, list or" + "tuple."
                )

            if isinstance(is_output, (list, tuple)):
                if len(is_output) != sig_outputs_len:
                    message = (
                        "If argument 'is_output' is a list, then it "
            ...
            for val in is_output:
                if not isinstance(val, bool):
                    raise ValueError(
                        "All elements of 'is_output' must be"
                        + " of type bool."
                    )
        ...
        if keep is not None:
            if not isinstance(keep, (bool, list, tuple)):
                raise ValueError(
                    "Argument 'keep' must be bool, list or" + "tuple."
                )

            if isinstance(keep, (list, tuple)):
                if len(keep) != sig_outputs_len:
                    message = (
                        "If argument 'keep' is a list, then it must "
        ...
        if persist is not None:
            if not isinstance(persist, (bool, list, tuple)):
                raise ValueError(
                    "Argument 'persist' must be bool, list or tuple."
                )

            if isinstance(persist, (list, tuple)):
                if len(persist) != sig_outputs_len:
                    message = (
                        "If argument 'persist' is a list, then it "
                        "must have the same length as 'outputs'."
                    )
                    raise ValueError(message)
            ...
            for index, node in enumerate(outputs):
                ...
                if not isinstance(node, self.signature[1][index]):
                    message = f"Output at position {index} is not compatible."
                    raise ValueError(message)
        ...
        if len(inputs) > 0:
            ...
            for key in ikeys:
                if key not in self.pipeline.nodes_up[self.key]:
                    message = f"Unknown input {key} for transition {self.key}."
                    raise ValueError(message)
