...
class AnnotationList(list):
    ...
    def add(
        ...
        if annotation is None:
            annotation = Annotation(
                geometry=geometry, labels=labels, metadata=metadata, id=id
            )
    ...
    def buffer(self, buffer=None, **kwargs):
        annotations = [
            annotation.buffer(buffer=buffer, **kwargs) for annotation in self
        ]
        return AnnotationList(annotations)

    def apply(self, func):
        annotations = [func(annotation) for annotation in self]
        return AnnotationList(annotations)

    def filter(self, func):
        """Return new AnnotationList with filtered annotations."""
        annotations = [annotation for annotation in self if func(annotation)]
        return AnnotationList(annotations)


class AnnotatedObjectMixin:
    def __init__(self, annotations=None, filter_annotations=True, **kwargs):
    ...
    def to_dict(self):
        return {"annotations": self.annotations.to_dict()}
