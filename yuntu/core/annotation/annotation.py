...
class Annotation(ABC):
    ...
    def __init__(self, labels=None, id=None, metadata=None, geometry=None):
    ...
    def __repr__(self):
        args = {"labels": repr(self.labels), "geometry": repr(self.geometry)}
    ...
    def to_weak(self):
        return WeakAnnotation(
            labels=self.labels, id=self.id, metadata=self.metadata
        )
...
class Annotation2DMixin(
    TimeIntervalAnnotationMixin, FrequencyIntervalAnnotationMixin
):
    def to_bbox(self):
        data = self._copy_dict()
...
class LineStringAnnotation(Annotation2DMixin, Annotation):
    ...
    def __init__(self, vertices=None, wkt=None, **kwargs):
        if "geometry" not in kwargs:
            kwargs["geometry"] = geom.LineString(wkt=wkt, vertices=vertices)
