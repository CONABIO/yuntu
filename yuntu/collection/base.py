...
from yuntu.core.database.base import ImageDatabaseManager
from yuntu.core.database.base import TimedImageDatabaseManager
...
class ImageCollection:
    ...
    media_class = Image
    annotation_class = Annotation
    db_manager_class = ImageDatabaseManager
    ...
    def __getitem__(self, key):
        queryset = self.images()
        if isinstance(key, int):
            return self.build_image(queryset[key:key + 1][0])

        return [self.build_image(image) for image in queryset[key]]

    def __iter__(self):
        for recording in self.images():
    ...
    def get(self, key, with_metadata=True):
        record = self.images(lambda rec: rec.id == key).get()
        return self.build_image(record, with_metadata=with_metadata)
    ...
    def get_image_dataframe(
        ...
        if limit is None:
            ...
            query_slice = slice(offset, offset + limit)
        images = self.images(query=query)[query_slice]
    ...
    def update_recordings(self, query, set_obj):
        """Update matches."""
        return self.db_manager.update(query, set_obj, model="images")
    ...
    def delete_images(self, query):
        """Delete matches."""
        return self.db_manager.delete(query, model='image')
    ...
    def images_model(self):
        return self.db_manager.models.image
    ...
    def images(self, query=None, iterate=True):
    ...
    def build_image(self, image, with_metadata=True):
        ...
        metadata = image.meta_arr if with_metadata else None
...
class TimedImageCollection(ImageCollection):
    """Time aware collection."""
    db_manager_class = TimedImageDatabaseManager
