    ...
    def __call__(self, *args):
        ...
        try:
            ...
            if self.methodName == "getSignals":
                result = [{"id": r["id"], "md5":r["md5"],
                           "signal":r["signal"].tolist()} for r in result]
            elif self.methodName == "getSpecs":
                result = [{"id": r["id"],
                           "md5": r["md5"],
                           "freqs": r["freqs"].tolist(),
                           "spec":r["spec"].tolist()} for r in result]

            self.response = Response(response=json.dumps(
...
class yuntuServer(object):
    def __init__(self,
        ...
        if collection.db.connPath == ":memory:":
            ...
            self.db = embeddedDb(collection.db.name,
                                 collection.db.dirPath,
                                 overwrite=False)
        self.db.close()
        ...
        if self.addCollectionEndpoints():
            ...
            raise ValueError(
                "Something went wrong while initializing flask server...")

    def addEndPoint(self,
                    endpoint,
                    endpointName,
                    methodName):
        self.app.add_url_rule(endpoint, endpointName, colAction(
            methodName, self.db, self.mediaDir))
